    ...
    while (iter <= maxiter && fgb>=tol)
        for i = 1:np     
            ...
            if typeof(cub)==TetSymCub{T}
                vg = copy(xp[i,1:cub.numparams])
                ng = 0
                eps_param = 1e-4
                vS31 = vg[1:cub.numS31]
                for k = 1:length(vS31)                
                    if (xp[i,ng+k] >= 1.0)
                        xp[i,ng+k]=1-eps_param
                    end
                    # println("=========S31========")
                end
                ng+=cub.numS31 
                ...
                for k = 1:2:length(vS211)
                    if ((xp[i,ng+k]+xp[i,ng+k+1]/2)>= 1.0)
                        while ((xp[i,ng+k]+xp[i,ng+k+1]/2)>= 1.0)
                            xp[i,ng+k]-= eps_param
                            xp[i,ng+k+1]-= eps_param
                            # println("=========S211========")
                        end
                ...
                for k = 1:3:length(vS1111)
                    if ((xp[i,ng+k]+xp[i, ng+k+1]+xp[i,ng+k+2])/2 >= 1.0)
                        while ((xp[i,ng+k]+xp[i, ng+k+1]+xp[i,ng+k+2])/2 >= 1.0)
                            xp[i,ng+k]-= eps_param
                            xp[i,ng+k+1]-= eps_param
                            xp[i,ng+k+2]-= eps_param
                            # println("=========S1111========")
                        end
                    end
                end     
...
function pso_weight(fun::Function, ne::Int, cub::SymCub{T}, q::Int, mask::AbstractArray{Int64,1}; np::Int64=10, xinit::Array{T}=[], 
    xL::T=convert(T, 0.0), xR::T=convert(T, 2.0), delta1::Float64=1e-2, delta2::Float64=1e-2,
    maxiter::Int64=1000, tol=10*eps(typeof(real(one(T)))), save_iter=false, verbose=0) where {T}
    ...
    for i = 1:np
        SymCubatures.setweights!(cub, x[i,:])
        fp[i,1] = norm(fun(cub, q, compute_grad=compute_grad)[1])
        xpb[i,:] = x[i,:]
    end
    fpb = copy(fp)          # personal best function values
    ...
    x_all=[]
    if save_iter
        SymCubatures.setweights!(cub, xgb)
        fxgb = norm(fun(cub, q, compute_grad=compute_grad)[1])
        push!(f_all,fxgb)
        push!(x_all,xgb)
    end
    ...
    while (iter <= maxiter && fgb>=tol)
        for i = 1:np     
            ...
            # evaluate objective function for the new particle position
            SymCubatures.setweights!(cub, xp[i,:])
            fxp=norm(fun(cub, q, compute_grad=compute_grad)[1])
            fp[i] = fxp
...
function pso(fun::Function, ne::Int, cub::AsymCub{T}, q::Int, mask::AbstractArray{Int64,1}; np::Int64=10, xinit::Array{T}=[], 
    xL::T=convert(T, 0.0), xR::T=convert(T, 2.0), delta1::Float64=1e-2, delta2::Float64=1e-2,
    maxiter::Int64=1000, tol=10*eps(typeof(real(one(T)))), save_iter=false, verbose=0) where {T}
    ...
    for i = 1:np
        # AsymCubatures.setparams!(cub, x[i,:][1:cub.numparams])
        AsymCubatures.setweights!(cub, x[i,:][cub.numparams+1:end])
        fp[i,1] = norm(fun(cub, q, compute_grad=compute_grad)[1])
        xpb[i,:] = x[i,:]
    end
    fpb = copy(fp)          # personal best function values
    ...
    x_all=[]
    if save_iter
        # AsymCubatures.setparams!(cub, xgb[1:cub.numparams])
        AsymCubatures.setweights!(cub, xgb[cub.numparams+1:end])
        fxgb = norm(fun(cub, q, compute_grad=compute_grad)[1])
        push!(f_all,fxgb)
        push!(x_all,xgb)
    end
    ...
    while (iter <= maxiter && fgb>=tol)
        for i = 1:np     
            ...
            # evaluate objective function for the new particle position
            # AsymCubatures.setparams!(cub, xp[i,:][1:cub.numparams])
            AsymCubatures.setweights!(cub, xp[i,:][cub.numparams+1:end])
            fxp=norm(fun(cub, q, compute_grad=compute_grad)[1])
            fp[i] = fxp
            ...
            for i = 1:length(vS31)                
                if (v[ng+i] >= 1.0)
                    ...
                    while (v[ng+i] >= 1.0&& alphaS31>0)
                        ...
                        v += alphaS31*dv
                        # println("=====================", "alpha=", alphaS31, ":  v=", v[ng+i])
                    end
                end
            end
            ng+=cub.numS31 
            ...
            for i = 1:length(vS22)
                if (v[ng+i] >= 1.0)
                    ...
                    while (v[ng+i] >= 1.0 && alphaS22>0)
                        ...
                        v += alphaS22*dv
                        # println("=====================S22==================", "alpha=", alphaS22, ":  v=", v[ng+i])
                    end
                end
            end
            ng+=cub.numS22 
            ...
            for i = 1:2:length(vS211)
                if ((v[ng+i]+v[ng+i+1]/2)>= 1.0)
                    ...
                    while ((v[ng+i]+v[ng+i+1]/2)>= 1.0 && alphaS211>0)
                        ...
                        v += alphaS211*dv
                        # println("=====================S211==================", "alpha=", alphaS211, ":  v=", v[ng+i])
                    end
                end
            end
            ng+=2*cub.numS211 
            ...
            for i = 1:3:length(vS1111)
                if ((v[ng+i]+v[ng+i+1]+v[ng+i+2])/2 >= 1.0)
                    ...
                    while ((v[ng+i]+v[ng+i+1]+v[ng+i+2])/2 >= 1.0 && alphaS1111>0)
                        ...
                        v += alphaS1111*dv
                        # println("=====================S1111==================", "alpha=", alphaS1111, ":  v=", v[ng+i])
                    end
                end
            end     
        end
...
* `iter`: number of itrations
"""
function levenberg_marquardt(fun::Function, cub::AsymCub{T}, q::Int64, mask::AbstractArray{Int64,1}; xinit::Array{T}=[], 
    xL::T=convert(T, 0.0), xR::T=convert(T, 2.0), nu=1000.0, maxiter::Int64=1000, tol=10*eps(typeof(real(one(T)))), verbose=0) where{T}

    Jac = AsymCubatures.calcjacobian(cub)
    ...
    for k = 1:maxiter
        ...
        if ((res < tol) && (minimum(v[cub.numparams+1:end])>1e-10))
            return res, v, k
        end
        ...
        eps = 1e-4
        for i=cub.numparams+1:length(axes(v,1))
            if v[i]<0.0
                v[mask] -= alpha*dv[mask]
                alpha = (eps - v[i])/(dv[i])
                v[mask] += alpha*dv[mask]
            end
        end
